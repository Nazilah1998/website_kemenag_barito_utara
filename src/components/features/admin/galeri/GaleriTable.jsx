  ...
  return (
    <div className="grid grid-cols-2 gap-3 sm:gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      {items.map((item, index) => (
        <article key={item.id} className="w-full group relative aspect-[3/4] overflow-hidden rounded-xl sm:rounded-2xl border border-slate-100 bg-white shadow-sm transition-all hover:shadow-2xl dark:border-slate-800 dark:bg-slate-900">
          <Image
            src={item.image_url}
            alt="Gallery Item"
            fill
          ...
          />
          <div className="absolute inset-0 bg-linear-to-t from-slate-950/80 via-transparent to-transparent opacity-100 sm:opacity-0 transition-opacity sm:group-hover:opacity-100" />

          {/* Actions Overlay */}
          <div className="absolute inset-0 flex flex-col justify-end p-3 sm:p-4 translate-y-0 opacity-100 sm:translate-y-4 sm:opacity-0 transition-all duration-300 sm:group-hover:translate-y-0 sm:group-hover:opacity-100">
            <div className="flex items-center justify-between gap-2 sm:gap-3">
              <div className="flex-1">
                <p className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-white/60">
              ...
              <div className="flex gap-1.5 sm:gap-2">
                <button
                  onClick={() => onEdit(item)}
                  className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-lg sm:rounded-xl bg-white text-slate-900 shadow-xl transition hover:bg-emerald-500 hover:text-white dark:bg-slate-800 dark:text-white dark:hover:bg-emerald-600"
                >
                ...
                <button
                  ...
                  className="flex h-7 w-7 sm:h-9 sm:w-9 items-center justify-center rounded-lg sm:rounded-xl bg-white text-rose-600 shadow-xl transition hover:bg-rose-600 hover:text-white disabled:opacity-50 dark:bg-slate-800 dark:hover:bg-rose-600"
                >
                  {deletingId === item.id ? (
