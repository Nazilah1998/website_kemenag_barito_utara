    ...
    <section className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-100 px-4 py-12 dark:bg-slate-950 sm:px-6 lg:px-8">
      ...
      <div className="relative w-full max-w-[440px] animate-in fade-in zoom-in duration-500">
        <div className="mb-10 flex flex-col items-center text-center">
          <Link
            href="/"
            className="group mb-8 transition-transform hover:scale-110"
          >
            <Image
        ...
        <div className="rounded-[2.5rem] border-2 border-white bg-white/90 p-8 shadow-2xl backdrop-blur-xl dark:border-white/5 dark:bg-slate-900/90 sm:p-10">
          {f.step === 1 && (
            ...
            >
              ...
              <button
                type="submit"
                disabled={f.submitting || !f.email.trim() || !f.turnstileToken}
              ...
              >
                <span className="relative z-10">
                  {f.submitting ? "Memverifikasi..." : "Lanjutkan Verifikasi"}
                </span>
          ...
          {f.step === 2 && (
            <form
              onSubmit={f.handleResetPassword}
              className="space-y-6 animate-in slide-in-from-right-4 duration-300"
            >
              <div className="space-y-4">
                <p className="text-[11px] font-bold uppercase tracking-widest text-emerald-700 dark:text-emerald-400 text-center">
                  Tahap 2: Perbaharui Password
                </p>

                <div className="group">
                  <label className="mb-2 block text-[9px] font-black uppercase tracking-[0.25em] text-slate-400">
                    Password Baru
                  </label>
                  ...
                  {f.password && (
                    <div className="mt-3 space-y-2 animate-in fade-in slide-in-from-top-1 duration-300">
                      <div className="flex items-center justify-between">
                        ...
                        <span
                          className={`text-[9px] font-black uppercase tracking-widest ${f.strength.score === 1 ? "text-rose-500" : f.strength.score === 2 ? "text-amber-500" : "text-emerald-500"}`}
                        >
                          {f.strength.label}
                        </span>
                      </div>
              ...
              <button
                type="submit"
                disabled={f.submitting || !f.password || !f.confirmPassword}
              ...
              >
                <span className="relative z-10">
                  {f.submitting ? "Memperbarui..." : "Simpan Password Baru"}
                </span>
                <div className="absolute inset-0 -translate-x-full bg-linear-to-r from-transparent via-white/10 to-transparent transition-transform duration-500 group-hover:translate-x-full" />
              </button>
            ...
            <div className="text-center space-y-6 animate-in zoom-in duration-300">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm font-medium text-slate-400 leading-relaxed">
                  Password Anda telah diperbarui. Silakan gunakan password baru
                  untuk masuk ke Panel Kendali.
                </p>
              </div>
              <Link
                href="/admin/login"
                className="inline-flex h-14 w-full items-center justify-center rounded-xl bg-slate-900 text-xs font-black uppercase tracking-[0.25em] text-white transition-all hover:bg-slate-800 active:scale-[0.98] dark:bg-white dark:text-black dark:hover:bg-slate-200"
              >
                Kembali ke Login
              </Link>
            </div>
            ...
            >
              <div className="space-y-4">
                ...
                <div className="group">
                  <label
                    htmlFor="otp"
                    className="mb-2 block text-center text-[9px] font-black uppercase tracking-[0.25em] text-slate-400"
                  >
              ...
              <button
                type="submit"
                disabled={f.submitting || f.otp.length < 6}
                className="group relative flex h-14 w-full items-center justify-center overflow-hidden rounded-xl bg-emerald-600 text-xs font-black uppercase tracking-[0.25em] text-white transition-all hover:bg-emerald-700 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400"
              >
                <span className="relative z-10">
                  {f.submitting ? "Memverifikasi..." : "Verifikasi OTP"}
                </span>
                <div className="absolute inset-0 -translate-x-full bg-linear-to-r from-transparent via-white/10 to-transparent transition-transform duration-500 group-hover:translate-x-full" />
              </button>
          ...
          {f.step !== 3 && f.step !== 4 && (
            <div className="mt-8 flex flex-col items-center gap-4 text-center">
              ...
              <Link
                href="/admin/login"
                className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-700 dark:text-emerald-400"
              >
                Batal & Kembali
              </Link>
            </div>
...
function StatusAlert({ type, message }) {
  ...
  return (
    <div
      className={`flex items-start gap-3 rounded-2xl border-2 p-4 animate-in slide-in-from-top-2 ${isSuccess ? "border-emerald-100 bg-emerald-50 text-emerald-700 dark:border-emerald-900/30 dark:bg-emerald-950/20 dark:text-emerald-400" : "border-rose-100 bg-rose-50 text-rose-700 dark:border-rose-900/30 dark:bg-rose-950/20 dark:text-rose-400"}`}
    >
      <div
        className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${isSuccess ? "bg-emerald-500" : "bg-rose-500"} text-white`}
      >
        <svg
          viewBox="0 0 24 24"
          className="h-3 w-3"
