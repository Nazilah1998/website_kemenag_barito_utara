      ...
      <div className="relative z-10 flex flex-col lg:flex-row items-center gap-10 p-8 lg:p-12">
        ...
        <div className="relative shrink-0">
          ...
          <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1.5 rounded-full bg-gradient-to-r from-amber-500 to-amber-400 px-4 py-1.5 shadow-lg shadow-amber-900/40">
            ...
            <span className="text-[9px] font-black tracking-widest text-white uppercase">Kepala Kantor</span>
          </div>
        </div>
        ...
        <div className="flex-1 text-center lg:text-left">
          ...
          <p className="mt-1.5 text-sm font-semibold text-white/60">{pejabat.judul}</p>
          ...
          <div className="mt-5 inline-flex items-center gap-2.5 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm px-5 py-3">
            <BadgeIcon />
            <span className="font-mono text-xs text-white/80 tracking-wide">NIP. {pejabat.nip_kepala || "-"}</span>
          </div>
      ...
      <div className="flex flex-1 flex-col items-center gap-5 p-7 text-center">
        ...
        {pejabat.deskripsi && (
          <div className="w-full text-left border-t border-slate-100 dark:border-slate-800 pt-4 mt-1">
            <p className={`text-xs text-slate-500 dark:text-slate-400 leading-relaxed ${!expanded && isLong ? "line-clamp-2" : ""}`}>
              {pejabat.deskripsi}
            </p>
            {isLong && (
              <button
...
// ─── MAIN COMPONENT ──────────────────────────────────────────────────────────
export default function ProfilPejabatUI({ breadcrumb, kepalaKantor, pejabatList }) {
  return (
    <>
      <PageBanner
      ...
      <div className="w-full px-6 sm:px-10 lg:px-16 xl:px-20 py-12 lg:py-16 space-y-14">
          ...
          <section>
            <div className="mb-8">
              ...
              <p className="text-center text-sm text-slate-500 dark:text-slate-400 mt-3">
                Para pejabat yang memimpin bidang dan seksi di lingkungan Kemenag Barito Utara
              </p>
            </div>

            <div className="flex flex-wrap justify-center gap-6">
              {pejabatList.map((p, i) => (
                <div key={p.id} className="w-full sm:w-[calc(50%-12px)] lg:w-[calc(33.333%-16px)] xl:w-[calc(25%-18px)] flex">
        ...
        <div className="rounded-[2rem] border border-slate-100 bg-slate-50/80 dark:border-slate-800 dark:bg-slate-900/50 p-8 flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="flex-1 text-center sm:text-left">
            <p className="text-sm font-bold text-slate-900 dark:text-white">Informasi Kepegawaian</p>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
              Data profil pejabat diperbarui secara berkala. Untuk informasi lebih lanjut mengenai kepegawaian,
              silakan menghubungi <strong>Sub Bagian Tata Usaha</strong> Kemenag Barito Utara.
            </p>
          </div>
          <Link
            href="/kontak"
